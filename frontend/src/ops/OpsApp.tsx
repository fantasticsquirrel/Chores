...
function OpsShell(): ReactElement {
  ...
  return (
    <div className="ops-shell">
      ...
      <p className="ops-identity">Signed in as {session?.user.email} · {session?.user.role === "OWNER" ? "Platform owner" : "Support"}</p>
...
function OpsHouseholdDetailPage(): ReactElement {
  ...
  useEffect(() => {
    ...
    void opsApi.listHouseholdEvents(id).then((value) => { if (active) setEvents(value); }).catch(() => undefined);
    void opsApi.listAuditEntries(id).then((value) => { if (active) setAudit(value); }).catch(() => undefined);
    return () => { active = false; };
  }, [id]);
