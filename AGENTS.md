...
- Recovery task `4.5` added dedicated frontend regression coverage in `frontend/src/api/client.fetch-binding-regression.test.ts` to ensure the shared `apiClient` binds `globalThis.fetch` at module initialization and prevents `Illegal invocation` failures.
- Recovery task `5.1` operational deploy loop: rebuild frontend with `npm run build`, restart the chore_tracking backend `uvicorn` process (`app.main:app` on `127.0.0.1:8501`), and verify runtime via `GET /chore-api/health`, `GET /chore-api/health/live`, `GET /chore-api/health/ready`, plus `GET /chore/` serving built SPA HTML.
- Recovery task `5.2` added seeded Playwright deploy smoke coverage under `frontend/e2e/smoke.spec.ts` with runner `npm run test:smoke --workspace frontend`; the runner seeds auth/chore data via `backend/scripts/seed_playwright_smoke.py` and must target the live DB path (`DATABASE_URL=sqlite:///$ROOT_DIR/data/chore_tracking.db`) to avoid writing fixtures into `frontend/data/chore_tracking.db`.
- Recovery task `5.3` expanded deployed Playwright auth checks in `frontend/e2e/smoke.spec.ts` to assert anonymous protected-route redirects, anonymous API `401 Not authenticated.` responses, and child-role `403 Forbidden.` responses on parent-only APIs.
- Recovery task `5.4` updated `README.md` with explicit auth setup (`/chore-api/auth/*` + CSRF header usage), a one-time first parent bootstrap flow via backend models, and a production deploy verification checklist including health/auth assertions plus Playwright smoke execution.
- Recovery task `5.5` completion-only updates must still run root quality gates (`npm run lint`, `npm test`) before checking the final plan box and appending `STATUS: COMPLETE` in `IMPLEMENTATION_PLAN.md`.
