...
import { FamilyCoreApiEndpoints, familyApiRoutes } from "./api-endpoints";
import type {
...
class PasswordResetHarness extends FamilyCoreApiEndpoints {
  ...
  protected post<TResponse, TBody>(path: string, body: TBody, _query?: RequestQuery): Promise<TResponse> {
...
describe("password reset shared API contract", () => {
  it("keeps the public reset routes under the deployed chore API prefix", () => {
    expect(`/chore-api${familyApiRoutes.passwordResetRequest}`).toBe("/chore-api/auth/password-reset/request");
    expect(`/chore-api${familyApiRoutes.passwordResetConfirm}`).toBe("/chore-api/auth/password-reset/confirm");
  });
  ...
  it("uses one opaque token, never a client-controlled reset row identifier", () => {
    ...
    expectTypeOf<PasswordResetRequestResponse>().toEqualTypeOf<{ detail: string }>();
    expectTypeOf<PasswordResetConfirmRequest>().toEqualTypeOf<{ token: string; new_password: string }>();
    expectTypeOf<PasswordResetConfirmResponse>().toEqualTypeOf<{ detail: string }>();
  });

  it("uses the shared POST transport for reset request and confirmation", async () => {
    const api = new PasswordResetHarness();
    const requestPayload: PasswordResetRequest = { email: "parent@example.com" };
    const confirmPayload: PasswordResetConfirmRequest = {
      token: "v1.random-row-id.a-valid-looking-opaque-proof-value-which-is-long",
      new_password: "a sufficiently long parent password",
    };

    await expect(api.requestPasswordReset(requestPayload)).resolves.toEqual({ detail: "accepted" });
    await expect(api.confirmPasswordReset(confirmPayload)).resolves.toEqual({ detail: "accepted" });
