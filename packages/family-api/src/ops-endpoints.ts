...
import type {
  ...
  OpsMfaRequest, OpsReasonedCommand, OpsReauthRequest, OpsSessionResponse, OpsSupportCase, OpsSupportNote,
} from "./ops-models";

export const opsApiRoutes = {
  auth: { login: "/auth/login", me: "/auth/me", logout: "/auth/logout", mfa: "/auth/mfa", reauthenticate: "/auth/reauth" },
  ...
  householdReconcile: (id: number) => `/households/${id}/billing/reconcile`,
...
export abstract class OpsApiEndpoints {
  ...
  verifyMfa(payload: OpsMfaRequest): Promise<OpsSessionResponse> { return this.post(opsApiRoutes.auth.mfa, payload); }
  reauthenticate(payload: OpsReauthRequest): Promise<OpsSessionResponse> { return this.post(opsApiRoutes.auth.reauthenticate, payload); }
  ...
  reconcileHousehold(id: number, payload: OpsReasonedCommand): Promise<OpsHouseholdDetail> { return this.post(opsApiRoutes.householdReconcile(id), payload); }
