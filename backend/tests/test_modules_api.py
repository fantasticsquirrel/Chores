...
def test_parent_admin_sees_all_default_modules(tmp_path: Path, monkeypatch) -> None:
    ...
    assert response.status_code == 200
    assert [module["key"] for module in response.json()["modules"]] == ["chores", "homeschool", "recipes", "support", "admin"]
...
def test_parent_admin_can_list_household_user_module_access(tmp_path: Path, monkeypatch) -> None:
    ...
    assert [module["key"] for module in payload[0]["modules"]] == ["chores", "homeschool", "recipes", "support", "admin"]
...
def test_parent_admin_can_create_additional_parent_user(tmp_path: Path, monkeypatch) -> None:
    ...
    with TestClient(app) as client:
        ...
        assert [module["key"] for module in payload["modules"]] == ["chores", "homeschool", "recipes", "support"]
...
def test_parent_admin_can_create_additional_parent_admin_user(tmp_path: Path, monkeypatch) -> None:
    ...
    assert [module["key"] for module in create_response.json()["modules"]] == ["chores", "homeschool", "recipes", "support", "admin"]
...
def test_parent_admin_can_override_user_module_access(tmp_path: Path, monkeypatch) -> None:
    ...
    assert response.status_code == 200
    assert [module["key"] for module in response.json()["modules"]] == ["chores", "recipes", "support", "admin"]
