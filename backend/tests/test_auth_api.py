...
def test_login_account_picker_lists_active_legacy_accounts_without_sensitive_fields(
    tmp_path: Path,
    monkeypatch,
) -> None:
    _configure_test_settings(tmp_path, monkeypatch)
    parent, _parent_password = _create_parent_user(email="dad")
    child, child_user, _child_password = _create_child_user(
        household_id=parent.household_id,
        child_name="A",
        child_email="child-1-1-generated@child.local",
    )
    ...
    with TestClient(app) as client:
        response = client.get("/chore-api/auth/login-accounts")

    assert response.status_code == 200
    assert response.headers["cache-control"] == "no-store"
    assert response.headers["referrer-policy"] == "no-referrer"
    accounts = response.json()
    assert [account["display_name"] for account in accounts] == ["dad", child.name]
    assert [account["mode"] for account in accounts] == ["parent", "child"]
    assert all(set(account) == {"account_token", "display_name", "mode"} for account in accounts)
    serialized = response.text
    assert child_user.email not in serialized
    assert "disabled-legacy-user" not in serialized
    assert "password_hash" not in serialized


def test_login_account_picker_token_still_requires_the_selected_accounts_password(
    tmp_path: Path,
    monkeypatch,
) -> None:
    _configure_test_settings(tmp_path, monkeypatch)
    parent, _parent_password = _create_parent_user(email="dad")
    _child, child_user, child_password = _create_child_user(
        household_id=parent.household_id,
        child_name="A",
        child_email="child-1-1-generated@child.local",
    )
    ...
    with TestClient(app) as client:
        accounts_response = client.get("/chore-api/auth/login-accounts")
        child_account = next(
            account
            for account in accounts_response.json()
            if account["mode"] == "child"
        )

        missing_password = client.post(
            "/chore-api/auth/login-account",
            json={"account_token": child_account["account_token"]},
        )
        wrong_password = client.post(
            "/chore-api/auth/login-account",
            json={
                "account_token": child_account["account_token"],
        ...
        tampered_token = client.post(
            "/chore-api/auth/login-account",
            json={
                "account_token": f'{child_account["account_token"]}x',
    ...
    assert wrong_password.json()["detail"] == "Invalid account or password."
    assert tampered_token.status_code == 401
    assert tampered_token.json()["detail"] == "Invalid account or password."
    assert login_response.status_code == 200
    assert login_response.json()["user"]["id"] == child_user.id
    assert login_response.json()["user"]["role"] == UserRole.CHILD.value
    assert "chore_tracker_session" in login_response.cookies
